  ...
  return (
    <>
      ...
      <section className="mx-auto max-w-content space-y-16 px-4 py-16 sm:px-6 sm:py-20">
        <div>
          <h2 className="font-display text-xl text-ink">Graduation Requirements</h2>
          <div className="mt-5">
            {gradDetailBySlug[entry.slug] ? (
              <GraduationDetailTable
                groups={gradDetailBySlug[entry.slug].groups}
                totalLabel={entry.summary.graduationTotal ? `${entry.summary.graduationTotal} credits` : null}
                lang="en"
              />
            ) : (
