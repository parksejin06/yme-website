...
export default async function NoticeDetailPageEn({ params }: { params: Promise<{ board: string; id: string }> }) {
  const { board, id } = await params;
  const key = BOARD_PARAM[board];
  if (!key) notFound();
  ...
  return (
    <PostDetail
      post={post}
      lang="en"
      backHref={listHref("en", key)}
      prev={prev ? { href: postHref("en", key, prev.sourcePostId), title: prev.title } : null}
      next={next ? { href: postHref("en", key, next.sourcePostId), title: next.title } : null}
    />
  );
}
